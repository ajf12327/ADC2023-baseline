...
The easiest way for me to understand the Ariel dataset is to stop thinking of the “training data” as one table.
...
The challenge training package contains spectra, metadata and known target values. The challenge test package contains spectra and metadata, but it does not contain the target values.
...
This means that there is not one complete “training row” stored in one place. I have to construct one modelling example by joining the relevant pieces using `planet_ID`.
...
My initial audit found that each of these arrays currently has 52 values.
...
If every planet uses the same wavelength grid and the same bin widths, I may not need to provide the wavelength and width arrays explicitly to the model. The position of each value in the 52-element spectrum would already encode the wavelength ordering.

This assumption still needs to be checked during WP2. If the wavelength grids differ between planets, I will need to revisit how wavelength information is represented.
...
The external challenge test auxiliary table contains the same columns for 685 planets.

These values are potential model inputs. I do not necessarily need to use every available metadata field.

Track A specifies that the initial modelling input should include selected stellar and planetary metadata that would be available at prediction time. The exact metadata subset is still a provisional decision.
...
This is only an illustration. The final metadata subset has not yet been fixed.

---

## `FM_Parameter_Table.csv`

`FM_Parameter_Table.csv` contains the seven simulator values that Track A will predict.
...
For Track A, the target vector for planet \(i\) is:
...
The core supervised-learning problem is therefore:
...
`TraceData.hdf5` contains many weighted retrieval samples for each planet:
...
Track A uses the simulator truth as the prediction target. It is not trying to reproduce the complete Bayesian posterior.

Training on the posterior traces is deliberately excluded from the light Track A scope. Therefore, for the current project:

```text
FM_Parameter_Table.csv    use for Track A targets
TraceData.hdf5            do not use for Track A model training
```

I will retain `TraceData.hdf5` because it belongs to the original challenge and could become relevant to Track B, but it is not needed for the core WP2 modelling dataset.

---

## Challenge training data versus Track A partitions
...
However, Track A will divide the 41,423 labelled examples in this package into four internal partitions:
...
Used for early stopping and limited model selection.
...
WP2 will create these four partitions using unique planet identifiers. Their identifier lists will be saved so that the split is reproducible and mutually exclusive.
...
I can make predictions for these 685 planets, but I cannot directly calculate:
...
For this reason, the external challenge test set cannot be used for the main Track A coverage evaluation.

To avoid ambiguity, I will use the following names consistently:
...
I will avoid referring to both the held-out labelled partition and the external challenge data simply as “test.”
...
## My essential mental model

For every labelled planet, WP2 needs to create and validate one complete planet-level record:
...
The purpose of the current data audit is to make sure that all four pieces belong to the same planet before I create splits, fit preprocessing transformations or train a model.
